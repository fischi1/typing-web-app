...
import { clamp, lerp } from "../../functions/mathHelper";
...
export class ProgressBar extends GameObject {
    ...
    update(gameContext: GameContext): void {
        this.currentHeight = lerp(
            this.currentHeight,
            this.actualHeight,
            gameContext.deltaTime * 30
        )
    ...
    draw() {
        drawRect(
            this.barActiveGraphics,
            this.activeColor,
            { x: this.position.x, y: this.position.y + this.currentHeight },
            { x: this.bounds.x, y: this.bounds.y - this.currentHeight }
        )
    }

    setValue(val: number) {
        if (!isNaN(val)) this.actualHeight = this.bounds.y * clamp(1 - val)
    }
}
