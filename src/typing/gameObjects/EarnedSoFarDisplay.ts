...
export class EarnedSoFarDisplay extends GameObject {
    ...
    constructor(diamondSprite: PIXI.Sprite, gameContext: GameContext) {
        ...
        this.earnedSoFarLabel = new Text(
            "Earned so far: ",
            basPos,
            0,
            textStyle.clone()
        ...
        this.earnedSoFarValue = new Text(
            "0",
            add(basPos, { x: 300, y: 0 }),
            0,
            textStyle
        )
        ...
        this.diamond.sprite.position = vecToPixiPoint(
            add(basPos, { x: 300, y: -15 })
        )
        ...
        else EarnedSoFarDisplay.instance = this
    }
    ...
    wordComplete: WordListenerFunction = (current, next) => {
        ...
        const pointsToAdd = Math.round(
            this.pointsPerLetter * MultiplierDisplay.instance.value
        )
