...
export type WordListenerFunction = {
    (currentWord: Word | null, nextWord: Word | null): void
}
...
export class TypeTracker extends GameObject {
    ...
    constructor(
        words: Word[],
        letterWidth: number,
        letterContainer: PixiContainer,
        gameDone: { (reason: GameResultReasonType): void }
    ) {
        super()
        this.words = words
        ...
        else TypeTracker.instance = this
    }
    ...
    typingListener: ListenerFunction = key => {
        ...
        if (
            this.lastLetter.index === this.letterIndex &&
            key === this.lastLetter.character
        )
            //last letter
            this.nextWord()
        else if (key === " ") {
            if (this.curWord.text === this.curInput) this.nextWord()
            else this.addLetter(" ")
        } else if (key.length === 1) {
            this.addLetter(key)
        } else if (key === "Backspace") {
            if (this.curInput.length) {
                this.curInput = this.curInput.substr(
                    0,
                    this.curInput.length - 1
                )
            } else {
    ...
    nextWord() {
        ...
        if (this.wordIndex < this.words.length) {
            if (this.wordIndex === 0)
                this.wordCorrectRegistered.forEach(func =>
                    func(null, this.words[this.wordIndex])
                )
            else
                this.wordCorrectRegistered.forEach(func =>
                    func(this.curWord, this.words[this.wordIndex])
                )
    ...
    updateCurrentWord() {
        ...
        if (i < this.curWord.letters.length) {
            //before the last letter for word
            curLetter = this.curWord.letters[i]
            subLetter = curLetter.subLetter
            this.letterIndex = curLetter.index
            if (subLetter) Cursor.instance.setPosition(subLetter.curPos)
        } else {
            ...
            if (subLetter && Cursor.instance)
                Cursor.instance.setPosition({
                    x: subLetter.curPos.x + this.letterWidth,
                    y: subLetter.curPos.y
                })
        }
        ...
        while (i < this.curWord.text.length) {
            ...
            if (subLetter) subLetter.setStatus("invisible")
