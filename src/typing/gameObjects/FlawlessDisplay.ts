...
export class FlawlessDisplay extends GameObject {
    ...
    constructor(gameContext: GameContext) {
        ...
        this.curLabel = new Text(
            "cur",
            add(basPos, { x: 85, y: 40 }),
            0,
            textStyle
        )
        this.curLabel.pixiText.tint = pixiColorHelper.gray
        this.maxValueLabel = new Text(
            "0",
            add(basPos, { x: 35, y: 80 }),
            0,
            textStyle
        )
        this.maxValueLabel.pixiText.anchor.x = 0.5
        this.curValueLabel = new Text(
            "0",
            add(basPos, { x: 110, y: 80 }),
            0,
            textStyle
        )
    ...
    init(gameContext: GameContext) {
        ...
        TypeTracker.instance.registerWordCorrectListener(
    ...
    wordCompleteCallback: WordListenerFunction = (currentWord, nextWord) => {
        ...
        if (!this.wordAlreadyWrong) this.curValue++

        this.wordAlreadyWrong = false

        if (this.maxValue < this.curValue) this.maxValue = this.curValue
