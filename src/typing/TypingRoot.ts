...
import { GameResultReasonType, GameResultType } from "../types/GameResultType"
...
import generateGOs, {
    generateLetterSprite,
    LetterGenerationParamsType
} from "./generateGOs"
        ...
        var setup = async (loader: PIXI.Loader, resources: any) => {
            ...
            var errorLetterPool = new ErrorLetterPool(
                generateGOs(
                    "########################################",
                    letterParams
                ) //str should be as long as the longest word
            )
            ...
            this.gameObjects.push(
                new Cursor(
                    generateLetterSprite("|".charCodeAt(0), letterParams)
                )
            )
            ...
            ;(resources.damageDisplay.texture
                .baseTexture as PIXI.BaseTexture).scaleMode =
                PIXI.SCALE_MODES.NEAREST
            ;(resources.dest1.texture
                .baseTexture as PIXI.BaseTexture).scaleMode =
                PIXI.SCALE_MODES.NEAREST
            ;(resources.dest2.texture
                .baseTexture as PIXI.BaseTexture).scaleMode =
                PIXI.SCALE_MODES.NEAREST
            ;(resources.dest3.texture
                .baseTexture as PIXI.BaseTexture).scaleMode =
                PIXI.SCALE_MODES.NEAREST
            this.gameObjects.push(
                new LivesDisplay(
