...
const Home : FC<{}> = () => {
    ...
    const lastGameResult = gameResultHistoryState.history.length === 0 ?
        null :
        gameResultHistoryState.history[gameResultHistoryState.history.length - 1];
    
    return (
        <Grid container justify="center" alignItems="center" style={{height: `calc(100vh - ${appbarHeight})`}}> 
            <Container maxWidth="md" style={{textAlign: "center"}}>
                <Typography variant="h4" style={{marginBottom: theme.spacing(3)}}>Hello <span style={{color: highlightColors.green}}>{userInfoState.username}</span>!</Typography>
                <div style={{marginBottom: theme.spacing(3.5)}}>
                    {!lastGameResult ? (
                        <Typography variant="h6">You haven't completed any lesson yet</Typography>
                    ): (
                        <>
                            <Typography variant="h6">You have completed <span style={{color: highlightColors.blue}}>{lessonCompletedInfo.completed} / {lessonCompletedInfo.amount}</span> lessons so far</Typography>
                            <Typography variant="h6"><span style={{color: highlightColors.blue}}><ReactTimeago date={lastGameResult.date}/></span> you completed your last lesson</Typography>
                        </> 
                    )}
                </div>
