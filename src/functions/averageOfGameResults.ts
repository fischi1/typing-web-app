...
const averageOfGameResults = (
    data: GameResultType[]
): Record<keyof MappedDataType, string> | null => {
    ...
    var reduced = data.reduce((left, right) => ({
        ...
        maxStreak:
            left.maxStreak < right.maxStreak ? right.maxStreak : left.maxStreak,
    ...
    return {
        accuracy:
            Math.round((reduced.accuracy / data.length) * 10000) / 10000 + "",
        dayDate: "",
        gemsEarned: "",
