...
export const init = () => {
    ...
    function setup(loader : PIXI.Loader, resources : any) {
        ...
        for (var i = 0; i < loremIpsum.length; i++) {
            ...
            char.x = curLineX;
            char.tint = PIXI.utils.rgb2hex([Math.random(), Math.random(), Math.random()]);
...
function loop(delta : number) {
    ...
    time += deltaS;
    colorChangeTime += deltaS;

    if(colorChangeTime >= 1) {
        bitmapFontArr.forEach(char => char.tint = PIXI.utils.rgb2hex([Math.random(), Math.random(), Math.random()]));
        colorChangeTime = 0;
    }
